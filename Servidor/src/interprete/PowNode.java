...
public class PowNode implements PSNode {

    private PSNode lhs;
    private PSNode rhs;

    public PowNode(PSNode lhs, PSNode rhs) {
        this.lhs = lhs;
        this.rhs = rhs;
    }
    ...
    public PSValue evaluate(long idPaquete, Calendar fecha, IInterpreter intepr) {
        ...
        if(a.isNumber() && b.isNumber()) {
            return new PSValue(Math.pow(a.asDouble(), b.asDouble()));
        }
    ...
    public boolean isAtomic(){
    	return rhs.isAtomic() && lhs.isAtomic();
    }
}